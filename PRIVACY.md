...
- 上传的不是明文，而是 **AES-GCM-256 密文**：密钥由你的口令用 PBKDF2 另派一次（专门的盐 `dsh-enc-v1:<用户名>:<服务端盐>`，与登录用的派生值不同），每个文件独立随机 IV，密文自带版本头 `{ e:1, alg, kdf, iv, ct }`。
- 密钥只放在**本标签页的 `sessionStorage`**（关掉标签页即失效），不写 `localStorage`；重开页面后需要重新输入一次口令「解锁」，口令**不会上传**。
- 后果：**我们（以及任何拿到 KV/Gist 的人）都读不到你的存档内容**；反过来，**忘掉口令 = 云端存档解不开**（本机那份还在，也可以随时用「导出存档文件」备份）。

### 冲突与频率

- 上传带乐观锁：云端那份若已被别的设备更新，会返回 `409 conflict`，客户端先拉取再重试——不会出现"后保存的把另一台设备的进度吃掉"。
- 自动同步有两段节流（先合并 8 秒内的连续保存，再保证距上次写云至少 60 秒），避免把 KV/Gist 当高频存储刷（KV 免费额度每天 1000 次写）。
