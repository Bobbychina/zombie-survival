...
import { zoneGate, zoneGateNote, gateYield } from './gate-core';   // M73：军事区/实验室的「生存」等级门槛
...
export function searchPoi(block: Block, sw: SaveWorld, deep: boolean): boolean {
  ...
  /* M73：军事管制区/实验室要「生存」等级 —— 不够则搜刮效率腰斩（产出下限 1，不锁死）。
     判定与 legacy searchZone 共用 gate-core，两条搜刮路径不许出现两套口径。 */
  const zg = zoneGate(zone, S.skills);
  const zNote = zoneGateNote(zg) ? ' ' + zoneGateNote(zg) : '';
  ...
  if (left <= 0) {
    ...
    const d = gateYield(scavYield(Math.max(1, Math.round(L.ri(1, 2) + block.danger * 0.6)), scavMulOf((L.S as any).body)), zg.mul);   // M73：门槛打折
    S.mat += d; L.tickVitals(0.5); L.sfx('loot');
    L.log('🧹 ' + poi.icon + poi.name + '已经被翻得底朝天，你只刮出 ' + d + ' 份材料。' +
      '（这一趟照样记进任务进度）' + (zg.gated ? zNote : ''), 'loot');
  ...
  L.hr();
  L.log((deep ? '🔦 你在' + poi.name + '深处翻找，每一秒都在赌命……' : '🔍 你搜索' + poi.name + '……') + zNote, 'narrative');
  ...
  switch (kind) {
    ...
    case 'item': {
      ...
      const n = id === 'ammo' ? L.ri(6, 14) : (Math.random() < 0.25 * scav * zg.mul ? 2 : 1);   // M68：手上有伤就难得翻到双份；M73：门槛同样压双份概率
      L.grant(id, n);
      L.sfx('loot');
    ...
    case 'mats': {
      ...
      const mg = gateYield(m, zg.mul);   // M73：军事区/实验室不够「生存」级 → 腰斩（下限 1 份）
      S.mat += mg; L.sfx('loot');
      L.log('🔩 你撬开一堆残骸，回收了 ' + mg + ' 份材料。' + scavNote(scav) + (zg.gated ? zNote : ''), 'loot');
      break;
    }
    case 'food': {
