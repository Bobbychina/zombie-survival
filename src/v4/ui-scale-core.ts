...
export const DEFAULT_PREFS: UiPrefs = { fs: 100, mapOpen: true };
...
export function writePrefs(store: PrefsStore | null | undefined, p: UiPrefs): void {
  if (!store) return;
  try { store.setItem(FS_KEY, JSON.stringify({ fs: clampFs(p.fs), mapOpen: !!p.mapOpen })); } catch { /* 无痕模式：忽略 */ }
}
...
/** 探针/调试：把当前状态交出去 */
export const scaleStatus = (p: UiPrefs) => ({ fs: clampFs(p.fs), label: fsLabel(p.fs), zoom: zoomOf(p.fs), mapOpen: !!p.mapOpen, key: FS_KEY });
