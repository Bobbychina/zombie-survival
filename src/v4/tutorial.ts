...
export const STEPS: TutStep[] = [  {
  ...
  {
    /* M32：地图搬进右上角悬浮窗了，文案跟着改；before 先回探索页 —— 读档进来可能停在背包/人体页，
       不切回去这一步的目标就找不到（会被静默跳过，实测跳过第 6/7 步就是这个原因）。 */
    sel: '#v4world',
    before: () => V4Tutorial.go('explore'),
    title: '那张地图：你在哪、能去哪',
    body: '每个方块是 1km² 的区块，<b>点亮的格子才能走</b>（先点一次看路线与花费，再点一次才出发）。'
      + '<br>地图默认住在右上角的<b>悬浮窗</b>里：按 <span class="mono">M</span>（或点标题栏的 ✕）折起来就只剩一条小条，'
      + '<b>任何页签都能打开它</b>——大字模式下它也不会挤掉你的操作卡片；想让它跟着页面一起排，'
      + '就在 ☰ → <b>显示</b> 里把「地图位置」切成<b>嵌入页内</b>。'
      + '<br>窗里的「🗺️ 本地地图 / 🌐 大区地图」可以切换：本地图看"这条街有什么"，大区图看"该往哪个方向跑"。'
      + '<br>危险度是<b>从家往外涨</b>的：家里和紧邻一圈是安全区（危险 1），越往外越危险、也越有好东西。',
  },
  {
    sel: '#view.v4-board > .v4board',
    before: () => V4Tutorial.go('explore'),
    title: '下面这些卡片：一天的活儿都在这',
    body: '「今日行动」是今天能干的事；「格子详情」是脚下这格有什么；「采集与拆解」「水体」「菜园」是日常产出；'
      + '「今夜」管睡觉；「旅途记录」是你走过的路。'
      + '<br>卡片现在是<b>整宽</b>的（地图搬走之后这一片全是操作区），一张卡只讲一件事——哪里缺东西，就去对应的卡里找。'
      + '<br>字太小？☰ 菜单 → <b>显示</b> 里有 100%~160% 五档字号（Ctrl + / Ctrl −）。',
  },
  {
