...
0. **M64 已完成（全量审计）**（用户："好好检查一下有没有bug之类的"）：独立 subagent 只读审计 + 浏览器对抗探针
   ...
   线上版上是 3 红 + 崩溃）、回归 20 支全绿。**三个已知限制记在 ROADMAP P2 第 8 条**（保存后立刻关页面可能丢
   最后一次写 / `fitRegion` 单位混用 / `crossings` 负值不 clamp）。
