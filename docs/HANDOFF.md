...
| 测试基线 | **单测 411 通过 / 探针 10 套**（见 §5） |
| 当前 HEAD | `ab65138 [AI] M33 第一批：教程沙盒…` ＋ 本轮的 M33 第二批（第 2 章 + `bridge` 记账修复） |
...
### M33 已完成两批：教程沙盒（章节壳 + 独立 iframe 沙盒 + 第 1、2 章）

用户拍板的四条：**① 沙盒完全隔离 ② 章节完成判定 = 目标清单全绿 ③ 章内无限重来、不惩罚
④ 分批做章节**（第一批「壳 + iframe + 第 1 章」，第二批加第 2 章）。

- `src/v4/sandbox-core.ts`（纯逻辑、可单测）：6 章章节表（**第 1、2 章可玩**，第 3~6 章 `ready:false`）、
  `evalChapter`（全绿才算过）、`mergeSticky`（**勾选单调**：吃饱了再睡一觉饿下去，不许把打好的勾收回去）、
  `snapOf`（iframe 出来的快照一律当不可信输入：含 stats 计数、背包、`load` 装填选择）、进度读写、`sandboxUrl`/`labFromSearch`。
- `src/v4/tutorial-lab.ts`（父页面）：整屏覆盖层 = 左列章节 + 中间 iframe + 右列目标清单；
  iframe 每 0.5 秒 postMessage 一份快照，父页面判绿打勾，全绿写本机进度 `zsv-lab-v1` + 「✅ 已通关」徽章。
- **第 1 章「生存基础」**：搜刮 2 处 / 深搜 1 次 / 饱食水分都到 80 / 睡进第 2 天（预设 `lab-survival-01`，开局 62/58）。
- **第 2 章「战斗与枪械」**：枪杀 1 只（`kills>=1 && ammoUsed>=1`）/ 近战杀 1 只 / 手动换一次装填弹种
  （预设 `lab-combat-01`：手枪 + 撬棍 + 9mm FMJ 24 + AP 8）。顺手修掉 `bridge.syncBack()` 那条
  **v4 战斗引擎从不记 `stats.ammoUsed`** 的账（统计页"消耗弹药"长期偏低）。
...
- 探针 `docs/_m33_probe.mjs` **35/35**（本地连跑两遍 + **线上 35/35**）：两章都能靠**真实操作**
  （点搜索/深搜/使用/回安全屋睡/点招式槽/点装填）把目标打绿并通关；**隔离实测**：玩完整轮后主档密文一字未变、
  父页面进度未变、localStorage 只多出进度那一个键、沙盒里 `saveGame/autosave` 也写不动主档。
- 上线：`f5e7aa0`（源码）→ HEAD 干净构建（`git archive` 到临时目录 + junction `node_modules`）→ `8ff31be`（产物）
  → pages `3f35d87`；线上实测两章可玩、隔离成立。

### 下一批

1. **M33 第三批**：第 3~6 章（人体与伤病 / 建造与据点 / 地图与大区 / 背包与制作）——
   章节表里已经留好位置（`ready:false`），补 `objectives` + `preset` 即可。第 2 章遗留的
   "用穿甲弹打死装甲目标"需要一个新计数器，想做就得先在 legacy 加统计。
2. **同仓多 agent**：这个仓库同时有别的会话在改 M34（地图摆法）/ M35（搜刮记账）/ M36（作弊码下线）。
...
3. **探针端口会被别人抢**：多个会话同时跑 `tools\serve.mjs --port 8791` 时会顺延端口，别人的"清理 node 进程"
...
_m32_probe 17/17 · _m32b_probe 19/19 · _m33_probe 35/35     单测 411/411
```
...
1. 起浏览器把 §5 的十套探针 + 单测跑一遍，确认基线（任何一条红都先查是不是探针自身的前置状态）。
