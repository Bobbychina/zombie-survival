...
1. **M33 已完成六章**，可选的收尾：按章解锁/建议顺序、第 2 章"穿甲弹打死装甲目标"的硬验证
   （要新计数器）、第 5 章"真的开出去跨一次区"的探针（慢且飘，价值一般）。
2. **同仓多 agent**：这个仓库长期有别的会话在改（M34 地图摆法 / M35 搜刮记账 / M36 作弊码下线 / M35 收尾…）。
...
4. **探针的 outDir 要先建好**：有几套老探针不会自己 `mkdir`，传一个不存在的目录会直接崩在写截图上
   （本轮回归就踩了，白跑四套）。
...
_m32_probe 17/17 · _m32b_probe 19/19 · _m33_probe 49/49     单测 415/415
```
