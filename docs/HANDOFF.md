...
0. **M66 已完成（ROADMAP P1 第 1 条：建筑内部地图）**（用户："开始吧"）：进楼不是一下搜刮 —— 卡片上多一颗
...
1. **M55 已完成并上线**（用户报的速通漏洞）：「不在家 + 一直点睡觉 + 遇僵尸就逃跑 = 直接速通」。
   ...
   ⚠️ 另一处**探针随机性**（M66 回归时定量过）：m55 第 ⑤ 条「吃了罐头+喝水之后连饿计数归零」会偶发红 ——
   当晚的天气/消耗掷点把饥渴又压回阈值以下，`thirstN` 重新计上就判失败。同一支探针在 **M65 产物**上
   连跑两次都是 8/9（同一条），在 M66 上跑三次红一次。要根治得把那晚的天气固定下来（像 m33 那样先钉前置状态）。
