...
0. **M71 已完成（用户：「商人系统你不如参考塔科夫的好感度/做任务解锁购买特定道具」）**：纯逻辑在 **`src/v4/trader-core.ts`**
   ...
   给 5%/10%/16% 折扣（`effRate = merchantRate() × priceMulOf(rep)`）。军需官要先架无线电。
   `S.rep` 进 `sanitizeSave` 白名单（否则读档好感清零）。弹窗顶部两行好感面板 + 锁定行写清差什么。
   证据：单测 **820/820**、`docs/_m71_trader_probe.mjs` **11/11**（本地 + 线上）、回归 17 支。
   ⚠️ 两个坑：① `refreshMerchant` 不在 window 上（探针用"关掉再开"）；② **别直接改 `c.need`** ——
   `ensureContracts` 每次归一化出新对象，外部改动会被丢掉，要推**指标计数器**。价签认 🔩 那枚 tag（第一个 `.tag` 是"剩余"）。
   上线：`3829fbe` → `5e9f32a`（731621 字节）、站点 `1a90f95`（731667 字节）。

1. **M70 已完成（用户报障：藏身处太单调 / 耗材单一难找 / 通关都造不出几个）**：先做**经济审计**再动手，
   审查全文在 **`docs/BASE-REVIEW.md`**（脚本 `E:\Files\archive\sessions\2026-09\scripts\base-econ.mjs`）。
   根因不是"数值低"而是结构：**通用「材料」完全不能建造**（产量最大却只能买车/找商人），
   四种硬建材（胶带 26.5 AP/件、化学品 16.2、汽油 14.0、芯片 11.5）来源窄且与弹药制作抢料，
   曲线 ×(1+lv×0.6) 第三级 2.2 倍 → 满级全设施 **2170 AP ≈ 310 天**（一局 100 天只有 1400 AP）。
   改法：① 据点新卡 **♻️ 回收台**（材料 → 建材 7 行，**每日额度**、工作台 Lv1 解锁、每级 +1 额度）；
   ② 曲线 ×0.35（第三级 1.7×）；③ 发电机芯片 4→3、无线电 3→2；④ `S.base.exDay/exUsed` 进 `sanitizeSave` 白名单
   （不加的话读档就重置额度 = 无限兑换）。
   复算：**2170 AP(310 天) → 316 AP(45 天)**；单测 **809/809**、`docs/_m70_probe.mjs` **10/10**（本地 + 线上）、
   回归 15 支（m17/m25/m33/m39/m46/m50/m53/m54/m61/m65/m66/m67/m68/m69/m70）。
   ⚠️ 探针踩的两个坑：① 断言里把"铁片件数"当成"材料点数"比（口径错）② 挑卡片挑到了「该建什么」建议卡
   （那里没有 `.tag`，要按 `h3` 认设施卡）。上线：`cec0cff` → `b77b5b0`（726813 字节）、站点 `0281a01`（726859 字节）。

1. **M69 已完成（硬核化路线图 · 短期第 1 项收尾）**：**审计又抓到一次自己的误判** —— DESIGN-HARDCORE 里写的是
   「感染链没有每日恶化与致死」，实际 legacy 早有 `S.infect` 链（35 警戒 / 60 爆发每晚 +2 / 100 死 / 抗生素 −25）。
   真问题是**两条链不咬合**：v4 部位伤的「感染伤口」与 `S.infect` 毫无关系，带着没清创的伤口过夜感染照样消退。
   M69 打通：`infectNight()`（纯函数）+ `window.__v4InfectNight` 钩子（legacy `sleepNight` 里那段账改走它）——
   伤口每晚 **+4/处**且不再消退；`treat(part,'anti')` = **一天一次**的压制（`suppressDay` 新字段，老档视为没吃）；
   爆发期（≥60）`tickBody` 让伤口康复暂停；人体页/HUD 一行总览（`infectionLine`）。
   证据：单测 **801/801**、`docs/_m69_probe.mjs` **10/10**（本地 + 线上；含真实 `sleepNight` 走一遍）、
   回归 13 支（m25/m33/m46/m55/m57/m58/m61/m64/m65/m66/m67/m68/m69）。
   ⚠️ 这次回归里 m33 崩过一次：`JSON.parse(undefined)` —— 探针的 `ev()` 在 CDP evaluate 超时后返回 undefined，
   它直接拿去 `JSON.parse` 就炸了。**单跑复测 70/70**，属探针容错问题（该学 m55 那套：超时重试或直接判红）。
   上线：`0c70bcb` → `275c9b9`（723222 字节）、站点 `cb7c49e`（723268 字节）。

1. **M68 已完成（用户硬核化路线图 · 短期第 1 项）**：用户给了《从「余烬」到硬核生存的终局》（对标塔科夫/RLCraft/MC）——
   文档与**逐条差距审计**收进 **`docs/DESIGN-HARDCORE.md`**（15 条主张 → 已有/半有/缺口 → 映射 M68~M77）。
   审计纠正两条前提：**分部位身体系统（M31）与 12 个技能+按行为经验都已存在**，真缺口是"伤没接进动作"与"技能不构成门槛"。
   M68 的改法：`scavMulOf(body)`（手臂 0.45~1，平滑，`scavYield` 保底 1 件）接进门口快搜的材料档/双份掉落/搜空刮料 +
   楼内房间掉落；`headVisionLoss(body)`（头 < 55% 或脑震荡）接进 `scoutRadius()`（少 1 圈，下限 1 圈）；
   人体页与 HUD 写明「搜刮产出 -x% · 视野 -1 圈」；坏档缺字段一律按满血算。
   证据：单测 **791/791**、`docs/_m68_probe.mjs` **8/8**（本地 + 线上；同一掷点 10 → 5 份材料、视野 2→1 圈且新点亮 14→9 格）、
   回归 11 支（m17/m21/m25/m33/m46/m61/m64/m65/m66/m67/m68）。
   上线：`5699882` → `6b11113`（720353 字节）、站点 `06ca488`（720399 字节）。
   ⚠️ 探针踩坑记：量"点亮了多少格"**别数世界实例的 `revealed` 总数**（实例一换就是假红，第一版出现 gain=−34）——
   要在同一次调用里取 before/after 的增量。

1. **M67 已完成（用户报障："在楼内需要有一个单独的行动日志，不然看不到搜到了啥"）**：M66 的平面图是**全屏覆盖层**，
   legacy 的 `#log` 被压在下面 —— 点完搜刮只看到房间变灰。现在平面图自带 **📋 楼内日志**（`#v4i-log`，放在房间列表**上方**，
   房间多也不会被滚出视野；样式复用 `.round-log`、配色另给在 `v4.css`），日志**按楼**存在内存里
   （`Map<区块key, Line[]>`，上限 80 条）→ 关弹窗、打完架、出去再进来都还在。内容与世界日志**双写**：
   进楼 → 搜了哪间 → 「📦 拿到 …」（纯函数 `groupLoot` 同名归并成一行）/ 这间是空的 → 撬开·用钥匙·硬踹（含掉血）→ ☠️ 埋伏。
   证据：单测 **781/781**、`docs/_m67_probe.mjs` **8/8**（本地 + 线上）、`_m66_probe` 复跑 17/17、回归 23 支全绿。
   上线：`aa7dc6b` → `9f8bd54`（718978 字节）、站点 `669c291`（719024 字节，站点 CHANGELOG 补了 M67 一行）。

1. **M66 已完成（ROADMAP P1 第 1 条：建筑内部地图）**（用户："开始吧"）：进楼不是一下搜刮 —— 卡片上多一颗
   「🚪 进楼搜房（N 间 · 1 行动力/间）」，进去是 **3~6 间房的平面图**（`src/v4/interior-core.ts` 6 套房间模板 ×
   **26 个宿主建筑**；平面图 = `hash(poiId+种子+区块)`，**同一栋楼每次一样**，防止退出重进刷新布局）。
   规则：每间 1 行动力且**照旧走 tallySearch + checkQuest/bountyTick/sideTick**（M35 的账不能断）；`left` 同步 -1；
   **锁着的/深处的房间抽掉落表的"稀有"那半**；门三档出路（撬棍安静撬 / 楼门钥匙直接开 / 硬踹 -3~14 生命且必定遭遇），
   钥匙是新道具 `roomkey`（从别的房间翻）。进度存 `sw.interiors["x,y"]`（跟 `left` 一样随区域冻结/摊平）→ 分两趟搜完。
   **⚠️ 探针抓到一个自伤 bug**：`ensureSaveWorld` 每次 render 都被调到，第一版实现每次都**重建** interiors 表 →
   平面图 UI 手里那份 `st` 成了孤儿，刚搜过的房间下一次 render 就"复原"（实测连搜三间、关掉再进只剩两间）。
   改成就地清洗、保留对象身份 + 单测钉住。证据：单测 **778/778**、`docs/_m66_probe.mjs` **17/17**（本地 + 线上）、
   回归 22 支全绿。上线：游戏仓库 `7caf6a5` → `f2d26ab`（716595 字节）、站点仓库 `44675f4`（716641 字节，
   顺带上了一份站点 CHANGELOG.md）。

1. **M65 已完成（M64 审计留下的三条尾巴）**（用户："修掉这三个"）：① 存档写入**串行化 + 可等待**
   （`save-vault` 内部 `chain`，新增 `V4Vault.flush()` / `pending()`；`saveGame()` 等落盘再打"已保存"；
   `visibilitychange(hidden)` / `pagehide` 各 flush 一次兜底）—— 真修，探针里"同一 tick 内导航"从"看运气"变成 111 必在；
   ② `fitRegion` / 窄卡堆叠阈值的单位统一（`px(v)=v/z` 全部折到布局像素 + apply 后重测 chrome）——
   **⚠️ 做完受控 A/B 判定 M64 那条登记是误诊**：1440×1000 与 1440×1500 × 字号 100%/160% 下修前修后同数字
   （格子 27px / 17.59px、窗内滚 0 / 123px、chrome 389 / 474），160% 变小是"chrome 屏幕像素翻倍 + 18px 点击下限"的必然结果；
   代码照改（语义干净 + 窄窗堆叠判定不再拿布局像素比屏幕阈值），但**不宣称有可测收益**；
   ③ `ensureSaveWorld` 里 `crossings = Math.max(0, …)`（手改 -5 → 载入 0，真修）。
   证据：单测 **763/763**、`docs/_m65_probe.mjs` **11/11**（本地 + 线上），回归 21 支全绿。

1. **M64 已完成（全量审计）**（用户："好好检查一下有没有bug之类的"）：独立 subagent 只读审计 + 浏览器对抗探针
   （`docs/_m63_audit_probe.mjs`，18/18），查出并修掉 6 个真 bug（外加 1 处自己引入的符号错误）：
   ① 战斗中换武器**吞弹药**（`switchWeapon` 没刷 `S.ammo` 镜像 → 8 发霰弹被删、ammoUsed 虚增 8）·
   ② 钉刺陷阱**打全场**（应只扎最前面那只）· ③ v4 战斗胜利**少记账**（干净胜利计数 + 两条成就 + 猎人奖励）·
   ④ 辐射病/部位伤的**命中·闪避惩罚是死代码**（`statMods().hit/dodge` 全仓没人读 → 现在由 `bridge.playerProfile`
   注入 `accPenalty` 与闪避）· ⑤ 键盘 5 与第 5 招式槽撞车（第 5 槽改 **Q**）· ⑥ 掉血没 clamp（负数生命）+
   重复开战留下两个战斗覆盖层。证据：单测 **758/758**、`docs/_m64_probe.mjs` **13/13**（同一支探针在未修的
   线上版上是 3 红 + 崩溃）、回归 20 支全绿。三个已知限制当时记在 ROADMAP P2 第 8 条，**已在 M65 处理完**（见下一条）。

1. **M62 已完成**（ROADMAP P0「沙盒与主档的内容同步」）：M56 的引诱器进了教学第 2 章 —— 新增第 5 条目标
   「打不过就用引诱器脱身」，判定读 `stats.decoyUses`（**真的引走了才 +1**：v4 引擎记 `b.stats.decoys` →
   battle-ui 增量落库；legacy 战斗同源；引不走既不消耗也不记账）。沙盒发 `decoy1 ×2 + decoy2 ×1`。
   证据：单测 **753/753**、`docs/_m33_probe.mjs` **70/70**（本地 + 线上，含"拿简易引诱器赶暴君"的反例）。

1. **M61 已完成**（用户报障「还差…太暗了 / 缺什么材料、几级工作台不够明显」）：一套缺口视觉三处共用 ——
   `.tag.short`（缺料红）/ `.tag.gate`（等级黄）/ `.reqmiss` 高亮行（够料走绿色 `.ok` 变体），
   配 `needTags` / `missLine` / `stationChip` 三个小工具（制作页 + 据点设施卡 + 该建什么 + 抢修防线 + 陷阱卡）。
   制作页现在的缺口行是「⛔ 材料不够：还差 污水 ×2」/「🔒 工作台 等级不够：需要 Lv.2，现在 Lv.1」。
   证据：单测 **744/744**、`docs/_m61_probe.mjs` **11/11**（本地 + 线上，含对比度测量：缺口行亮度 199 vs 灰字 123）。
   ⚠️ m33 探针第 2 章那条"用穿甲弹打死装甲丧尸"是**真随机战斗**，会偶发假红 —— 已加"最多 3 次重开"的硬化。

1. **M60 已完成**（ROADMAP P0「教程沙盒：跨区旅行真的走一遍」）：第 5 章目标③从"有车"改成**真的跨一次大区** ——
   `world.crossings` 只在 `switchRegion()` 真换图时 +1（老档补 0），目标判定用 `crossings >= 1`；
   沙盒头部新增 **🔒 按顺序解锁** 开关（`LabProgress.seq`，**默认关** = 六章都能直接练，打开后按 1→6 硬解锁，
   锁定章会说明怎么把开关关掉）。单测 **744/744**、`docs/_m33_probe.mjs` **66/66**。
   ⚠️ 探针里取区域 id 别用 `DEV.regions`（沙盒 iframe 的 DEV 形状与父页面不同，没有那一份）——
   从大区图格子的 `pickRegion('rX-Y')` 取（同时也是"UI 上真的点了得到"的事实来源）。

1. **M59 已完成**（用户报障「大区地图会一直抽搐」）：`fitRegion` 的高度预算原来取自**含网格自身**的量
...
1. **M55 已完成并上线**（用户报的速通漏洞）：「不在家 + 一直点睡觉 + 遇僵尸就逃跑 = 直接速通」。
   ...
   ⚠️ 另一处**探针随机性**（M66/M67 回归时定量过）：m55 偶发红，而且是**两条不同的断言**轮着红 ——
   ⑤「吃了罐头+喝水之后连饿计数归零」（当晚天气/消耗把饥渴又压回阈值以下，`thirstN` 重新计上）与
   「吃饱喝足时睡觉照常回血（据点地板 +12）」。实测：M65 产物 8/9 ×2（⑤）、M66 产物整批红一次（⑤）+ 单跑 9/9 ×2、
   M67 产物整批红一次（回血那条）+ 单跑 9/9 ×3 —— 判定为探针自身的状态/随机性，与 M66/M67 无关。
   要根治得像 m33 那样先把当晚的天气与前置状态钉死。
...
5. **新增玩法进沙盒清单**（M62 起，每批更新收尾前过一遍 —— 沙盒是唯一的"教学方法"，主档加了东西它不知道就等于没教）：
   - [ ] 这一批加的**道具 / 招式 / 系统**，沙盒哪一章该教？（加一条目标或改一条文案）
   - [ ] 判定口径能"点一下就算"吗？要就**再加一个硬计数器**（例：M48 `apKills`、M60 `crossings`、M62 `decoyUses`），
         并且**失败路径也能验**（引不走不消耗 / 有车没开过不算）。
   - [ ] 沙盒预设给的东西够不够走完这条目标（弹药 / 材料 / 油 / 道具），会不会被运气卡住？
   - [ ] 探针里加**正例 + 反例**（`docs/_m33_probe.mjs`），并复跑整套探针。
...
2. 按 `docs/ROADMAP.md` 往下做：**P0 已清空、P1 第 1 条（建筑内部地图）M66 已交付**，
   当前第一个是 P1「区域事件卡（进大区弹一张正在发生什么的卡 + 2~3 个选择）」。
