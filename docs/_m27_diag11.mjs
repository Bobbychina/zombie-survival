...
console.log(await ev(`(async () => {
  ...
  for (let i = 0; i < 18; i++) {
    if (!V4Tutorial.isOpen()) { log.push('closed@' + i); break }
    ...
    const title = bub ? bub.querySelector('.v4tut-hd b').textContent.slice(0, 12) : '-'
    log.push(V4Tutorial.step() + '{' + (sp ? 'spot' : 'nospot') + ',' + title + '}')
    V4Tutorial.next()
    await new Promise(x => setTimeout(x, 420))
  }
  return log.join(' | ')
})()`))
console.log('btn-menu 存在: ' + await ev(`!!document.getElementById('btn-menu')`) + ' | board: ' + await ev(`!!document.querySelector('#view.v4-board')`))
ws.close()
