...
for (const [w, h] of [[1920, 1080], [1880, 1080], [1707, 960], [2048, 1105], [2560, 1440]]) {
  ...
  console.log(`${w}x${h}  ` + await ev(`(() => {
    ...
    const world = document.getElementById('v4world'), cards = document.getElementById('v4cards')
    const wr = world.getBoundingClientRect(), cr = cards.getBoundingClientRect()
    const rows = [...document.querySelectorAll('#view.v4-board > *')].map(e => { const r = e.getBoundingClientRect(); return (e.id||e.className) + ':top=' + Math.round(r.top) })
    return JSON.stringify({ worldW: Math.round(wr.width), cardsW: Math.round(cr.width),
      cardsRight: Math.round(cr.right) + '(视口' + innerWidth + ')', cardsScrollX: cards.scrollWidth - cards.clientWidth,
