...
// 用法：node docs/_m13_quest_probe.mjs <cdpPort> <url> <outDir>
...
await sleep(400)
console.log('  剧情: ' + storyRes)
const SR = JSON.parse(storyRes)
ok('第 1 章目标达成 → 章节推进到 2，并写下剧情日志',
  SR.chapterAfter === 1 && SR.storyState.log.length >= 1, JSON.stringify({ ch: SR.chapterAfter, log: SR.storyState.log.length }))
ok('剧情日志面板与章节文案在位', SR.archive && /大故事/.test(SR.uiChapter), SR.uiChapter)
await shot('m13-story-after')
...
ok('第 3 天起板子上出现跨区委托（region: 指标）', !!FR.farTitle && /^region:/.test(FR.farMetric || ''), JSON.stringify(FR))
...
const farDone = await ev(`(() => {
  ...
  const mid = V4Quest.summary();
  V4Quest.tick();
  const after = V4Quest.summary();
  return JSON.stringify({ okAccept, far: far.title, region: far.region, regionAfter: mid.region,
    activeMid: mid.active.length, regionVisits: S.world.regionVisits,
    activeAfter: after.active.length, done: after.done, log: after.log.slice(-2) });
})()`)
await sleep(700)
...
ok('跨区到访计数写进存档（region: 指标才有判定依据）',
  (FD.regionVisits || {})[FD.region] >= 1, JSON.stringify(FD.regionVisits))
ok('到访即完成 → 委托结算', FD.activeAfter === 0 && FD.done >= 1, JSON.stringify({ active: FD.activeAfter, done: FD.done }))
