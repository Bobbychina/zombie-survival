...
await ev(`V4World.mapMode('region')`); await sleep(1600)
console.log('fitRegionDbg = ' + await ev(`JSON.stringify(window.__fitRegionDbg || null)`))
console.log('实测 cell = ' + await ev(`Math.round(document.querySelector('#v4world .rgrid .rcell2').getBoundingClientRect().width)`))
ws.close()
