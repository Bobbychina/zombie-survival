...
console.log(await ev(`(() => {
  ...
  const out = ['card clientH=' + card.clientHeight + ' offsetH=' + card.offsetHeight + ' rectH=' + Math.round(card.getBoundingClientRect().height)]
  let s = 0
  for (const el of Array.from(card.children)) {
    const cs = getComputedStyle(el); const r = el.getBoundingClientRect()
    const mt = parseFloat(cs.marginTop)||0, mb = parseFloat(cs.marginBottom)||0
    out.push('  - ' + (el.id||el.className).slice(0,24) + ' h=' + Math.round(r.height) + ' mt=' + mt + ' mb=' + mb + ' disp=' + cs.display + ' flex=' + cs.flexBasis)
    s += r.height + mt + mb
  }
  const cs = getComputedStyle(card)
  out.push('sum children=' + Math.round(s) + ' pad=' + cs.paddingTop + '/' + cs.paddingBottom + ' border=' + cs.borderTopWidth + '/' + cs.borderBottomWidth + ' rowGap=' + cs.rowGap)
  out.push('=> siblings()=' + Math.round(s + (parseFloat(cs.paddingTop)||0) + (parseFloat(cs.paddingBottom)||0) + (parseFloat(cs.borderTopWidth)||0) + (parseFloat(cs.borderBottomWidth)||0)))
  const wrap = card.querySelector('.wmapwrap')
  out.push('wrap clientH=' + wrap.clientHeight + ' scrollH=' + wrap.scrollHeight + ' offsetH=' + wrap.offsetHeight)
  return out.join('\\n')
})()`))
ws.close()
