...
ws.onmessage = (e) => { const m = JSON.parse(e.data); if (m.id && pending.has(m.id)) { pending.get(m.id)(m); pending.delete(m.id) }
  if (m.method === 'Runtime.consoleAPICalled') logs.push('[' + m.params.type + '] ' + (m.params.args||[]).map(a=>String(a.value??a.description??'')).join(' ').slice(0,200))
...
console.log('state: ' + await ev(`JSON.stringify({ V4Tutorial: typeof V4Tutorial, root: !!document.getElementById('v4tut'), ls: { done: localStorage.getItem('dsh.tutorial.done'), step: localStorage.getItem('dsh.tutorial.step') }, search: location.search, booted: typeof window.S !== 'undefined' })`))
console.log('logs:\n' + logs.slice(-8).join('\n'))
ws.close()
