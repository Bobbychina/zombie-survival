...
console.log(await ev(`(async () => {
  DEV.battle(['walker','runner'])
  await new Promise(r => setTimeout(r, 1200))
  V4Tutorial.start(true)
  await new Promise(r => setTimeout(r, 600))
  const log = []
  for (let i = 0; i < 14; i++) {
    if (!V4Tutorial.isOpen()) break
    const st = V4Tutorial.step()
    ...
    if (st === 11) {
      ...
      log.push('战斗界面=' + !!ov + ' 圈=' + (spr && orb ? Math.round(spr.left) + '..' + Math.round(spr.right) + ' vs ' + Math.round(orb.left) + '..' + Math.round(orb.right) : '-'))
      break
    }
    V4Tutorial.next()
